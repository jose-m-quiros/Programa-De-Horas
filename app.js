...
function generateWeeklyReport() {
  ...
  // Registrar fuente DejaVuSans con soporte UTF-8 (incluye simbolo colon colonés ₡)
  ...
  setF('bold', 16); doc.text('Reporte de Planilla Semanal', m+14, y+22);
  setF('normal', 10); doc.text('Semana: ' + weekLabel, m+14, y+40);
  y += 64;
  ...
  EMPLOYEES.forEach(function(emp) {
    ...
    doc.setTextColor(255,255,255);
    setF('bold', 11); doc.text(emp.name, m+10, y+16);
    setF('normal', 10); doc.text(fmt(emp.rate)+'/hr', W-m-10, y+16, {align:'right'});
    y += 24;
    ...
    rows.forEach(function(r, i) {
      ...
      setF('normal', 10); doc.text(r[0], m+6, y+12);
      doc.text(r[1], m+cw*1.5, y+12, {align:'center'});
      doc.text(r[2], m+cw*2.5, y+12, {align:'center'});
      doc.setTextColor(30,64,175);
      setF('bold', 10); doc.text(r[3], W-m-6, y+12, {align:'right'});
      y += 18;
    });
    ...
    doc.line(m, y, W-m, y);
    doc.setTextColor(100,116,139);
    setF('normal', 10); doc.text('Total: '+wHrs.toFixed(1)+' hrs', m+10, y+14);
    doc.setTextColor(5,150,105);
    setF('bold', 12); doc.text(fmt(sal), W-m-10, y+14, {align:'right'});
    y += 30;
  });
  ...
  doc.setTextColor(255,255,255);
  setF('bold', 12); doc.text('Total Planilla  -  '+grandHours.toFixed(1)+' hrs', m+14, y+22);
  setF('bold', 14); doc.text(fmt(grandTotal), W-m-14, y+22, {align:'right'});
